    ...
    internal class UpdateHandler : IUpdateHandler
    {
        ...
        private async Task OnInfoCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            outputBuilder.AppendLine($"{newlineSymbol}" +
                                $"*  Текущая версия программы 14.0.  Дата создания 26-11-2025{newlineSymbol}" +
                                $"   Реализовано подключение ORM  (ДЗ 15) {newlineSymbol}" +
                                $"*  Версия программы 13.0.  Дата создания 22-11-2025{newlineSymbol}" +
                                $"   Реализована модель БД (ДЗ 14) {newlineSymbol}" +
                                $"*  Версия программы 12.0.  Дата создания 20-11-2025{newlineSymbol}" +
                                $"   Реализована пагинация, просмотр выполненных задач, удаление задач через сценарии (ДЗ 13) {newlineSymbol}" +
                                $"*  Версия программы 11.0.  Дата создания 10-11-2025{newlineSymbol}" +
                                $"   Реализована работа cо списками для задач (ДЗ 12) {newlineSymbol}" +
                                $"*  Версия программы 10.0.  Дата создания 09-10-2025{newlineSymbol}" +
                                $"   Реализована работа cо сценариями и команда отмены сценария /cancel (ДЗ 11) {newlineSymbol}" +
                                $"*  Версия программы 9.0.  Дата создания 24-09-2025{newlineSymbol}" +
